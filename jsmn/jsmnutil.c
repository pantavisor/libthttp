...
static int iterate_json_item(const char *buf, jsmntok_t *tok, token_iter_f func,
			     void *data)
{
	int i;
	int c = 0;
...
int jsmnutil_parse_json(const char *buf, jsmntok_t **jsonv_out, int *jsons_out)
{
	jsmn_parser parser;
	int r, offset;
...
again:
	r = jsmn_parse(&parser, buf, strlen(buf), *jsonv_out, *jsons_out);

	if (r < 0) {
		if (r == JSMN_ERROR_NOMEM) {
			offset = *jsons_out;
			*jsons_out = *jsons_out * 2;
			*jsonv_out = realloc(*jsonv_out,
					     sizeof(jsmntok_t) * *jsons_out);
			memset(*jsonv_out + offset, 0,
			       sizeof(jsmntok_t) * offset);
			if (jsonv_out == NULL) {
				return 0;
			}
			goto again;
...
int jsmnutil_array_count(const char *buf, jsmntok_t *tok)
{
...
jsmntok_t **jsmnutil_get_array_toks(const char *buf, jsmntok_t *tok)
{
	jsmntok_t **arr;
...
int jsmnutil_object_key_count(const char *buf, jsmntok_t *tok)
{
...
jsmntok_t **jsmnutil_get_object_keys(const char *buf, jsmntok_t *tok)
{
	jsmntok_t **arr;
	int c;
...
jsmntok_t *jsmnutil_get_object_key_value(const char *buf, jsmntok_t *tok)
{
